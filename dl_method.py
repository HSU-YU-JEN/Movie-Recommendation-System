...
Original file is located at
    https://colab.research.google.com/drive/1Ght_Vv7lermlvIztK7Y4tSEEHApRZnLX
...
train = ratings.take(35_000) # creates a training dataset train by taking the first 35,000 elements from the shuffled dataset

# skip: Creates a Dataset that skips count elements from this dataset.
test = ratings.skip(35_000).take(8_188) # taking the next 8,188 elements
...
predicted_ratings = []
def predict_rating_number(user, movie):
    trained_movie_embeddings, trained_user_embeddings, predicted_rating = model({
          "userId": np.array([str(user)]),
          "original_title": np.array([movie])
      })
    #print("Predicted rating for {}: {}".format(movie, predicted_rating.numpy()[0][0]))
    predicted_ratings.append(predicted_rating.numpy()[0][0])

predict_rating_number(123,'The Wanderers'), predict_rating_number(123,'Kurz und schmerzlos'),
predict_rating_number(123,'Un long dimanche de fiançailles'), predict_rating_number(123,'Jezebel'),
predict_rating_number(123,'The Greatest Story Ever Told'), predict_rating_number(123,'Fools Rush In')
predict_rating_number(123,'The Bourne Supremacy'), predict_rating_number(123,'Young and Innocent')
predict_rating_number(123,'La révolution française')

history_ratings = [4,5,5,4,5,4,5,5,5]
print("history rating for 6 drama movies", history_ratings)
print("predict rating for 6 drama movies", predicted_ratings)

import matplotlib.pyplot as plt
from sklearn.preprocessing import StandardScaler

# Define a custom mapping function to compress ratings to the range [2, 3]
def compress_ratings(rating):
    # Example mapping: linearly map the original ratings to the range [2, 3]
    return 2 + (rating - min(history_ratings)) * (1) / (max(history_ratings) - min(history_ratings))

# Apply the mapping function to compress historical ratings
compressed_history_ratings = [compress_ratings(rating) for rating in history_ratings]

# Movie numbers
movies = list(range(1, 10))

# Plotting the compressed historical ratings on the graph
plt.plot(movies, compressed_history_ratings, marker='o', linestyle='-', label='Compressed Historical Ratings')
plt.plot(movies, predicted_ratings, marker='o', linestyle='-', label='Predicted Ratings')


# Adding title and labels
plt.title('Compressed Historical Ratings')
plt.xlabel('Movie Number')
plt.ylabel('Compressed Rating')
...
# Plotting the compressed historical ratings on the graph
plt.plot(movies, history_ratings, marker='o', linestyle='-', label='Historical Ratings')
plt.plot(movies, predicted_ratings, marker='o', linestyle='-', label='Predicted Ratings')


# Adding title and labels
plt.title('Ratings for User 1')
plt.xlabel('Movie Number')
plt.ylabel('Rating')
...
r = np.corrcoef(compressed_history_ratings, predicted_ratings)
r

import scipy.stats
scipy.stats.pearsonr(compressed_history_ratings, predicted_ratings)

"""So, le't examine **User 123** from historical data"""

ratings_df[ratings_df['userId'] == '123']

ratings_df[ratings_df['userId'] == '122']
